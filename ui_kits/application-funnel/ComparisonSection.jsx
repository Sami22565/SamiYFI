function ComparisonSection({ Badge }) {
  ...
  return (
    <section style={{ padding: '140px 0', background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      <div className="sy-split-cols" style={{ maxWidth: 1100, margin: '0 auto', padding: '0 32px', position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        <Reveal className="sy-split-left" style={{ paddingRight: 60, paddingTop: 8 }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--accent-primary)', borderRadius: '28px 6px 28px 6px', padding: 32, boxShadow: 'var(--shadow-glow-ember)', transform: 'rotate(-1.5deg)' }}>
        ...
        <div className="sy-split-badge" style={{ position: 'absolute', left: '50%', top: 4, transform: 'translateX(-50%)', font: '800 22px var(--font-display)', color: 'var(--text-muted)', background: 'var(--bg-surface)', padding: '4px 16px', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius-pill)', zIndex: 2 }}>VS</div>
        <Reveal className="sy-split-right" delay="0.1s" style={{ paddingLeft: 60, paddingTop: 8 }}>
          <div style={{ padding: 32, opacity: 0.75 }}>
