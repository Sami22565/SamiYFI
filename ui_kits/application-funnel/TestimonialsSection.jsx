function TestimonialsSection({ Badge, TestimonialCard }) {
  ...
  return (
    <section id="results" style={{ padding: '140px 0 140px' }}>
      ...
      <div style={{ display: 'flex', gap: 24, overflowX: 'auto', padding: '0 32px 12px', scrollSnapType: 'x mandatory' }}>
        {items.map((t, i) => (
          <Reveal key={t.name} delay={`${i * 0.08}s`} className="sy-testimonial-slide" style={{ flex: '0 0 380px', scrollSnapAlign: 'start' }}>
          <div className="sy-hover-lift" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: 32, height: '100%' }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
              {Array.from({ length: 5 }).map((_, k) => (
                <i key={k} data-lucide="star" style={{ width: 16, height: 16, color: 'var(--gold-500)', fill: 'var(--gold-500)' }}></i>
              ))}
            </div>
