function OfferSection({ Badge }) {
  ...
  return (
    <section id="mentorship" style={{ padding: '140px 0', background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
        ...
        <div>
          {items.map((item, i) => (
            <Reveal key={item.n} delay={`${i * 0.06}s`}>
            <div className="sy-hover-lift" style={{ display: 'grid', gridTemplateColumns: '80px 48px 1fr', gap: 24, alignItems: 'center', padding: '32px 24px', margin: '0 -24px', borderTop: i === 0 ? '1px solid var(--border-strong)' : '1px solid var(--border-subtle)', borderBottom: i === items.length - 1 ? '1px solid var(--border-strong)' : 'none', borderRadius: 'var(--radius-md)' }}>
